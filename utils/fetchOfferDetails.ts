...
const fetchOfferDetails = async (
  paymentId: number,
  setPayData: Dispatch<React.SetStateAction<PaymentData | null>>
) => {
  if (!paymentId) return;
  console.log(
    `fetchOfferDetails - Fetching details for Payment ID: ${paymentId}`
  );
  ...
  try {
    ...
    if (offerData.status === 200 && offerData.data.payment_id === paymentId) {
      setPayData(offerData.data); // Store offer details if the payment_id matches
